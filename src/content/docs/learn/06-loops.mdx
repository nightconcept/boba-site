...
The `for` loop is ideal when you want to repeat a block of code a specific number of times. The loop variable (`i` in the example below) and the range bounds (`1` and `5`) are always of type `int`. The `by 1` step is the default and can be omitted for simplicity.
